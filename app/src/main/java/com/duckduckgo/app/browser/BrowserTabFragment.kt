...
class BrowserTabFragment :
    ...
    private fun configureOmnibarQuickAccessGrid() {
        ...
        omnibarQuickAccessAdapter =
            createQuickAccessAdapter(originPixel = AppPixelName.FAVORITE_OMNIBAR_ITEM_PRESSED) { viewHolder ->
    ...
    private fun showCredentialsTooltip(
        url: String,
        credentials: List<Credentials>
    ) {
        Timber.e("showCredentialsTooltip: url=%s, numberSavedCreds=%d", url, credentials.size)

        savedCredentialsTooltipDialog?.dismiss()

        savedCredentialsTooltipDialog = CredentialsAutofillTooltipFragment.instance(url, credentials)
    ...
    inner class BrowserTabFragmentDecorator {
        ...
        private fun decorateToolbarWithButtons() {
            ...
            fireMenuButton?.setOnLongClickListener {
                ...
                viewModel.saveCredentials(url = url, username = "craig", password = "pwpwpwpw")
                viewModel.saveCredentials(url = url, username = "foo", password = "fakePassword")
                viewModel.saveCredentials(url = url, username = "bar", password = "dummyExtraLongPassword1234567890")
                true
            }
